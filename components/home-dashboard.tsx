...
import { buildTrainingLoopModel, type TrainingLoopAction } from '@/lib/training-loop'
...
export function HomeDashboard({ diagnosis, onboardingAnswers, onUpdateAirlinePreferences, onEditDiagnosis, onLogin, initialAccountOpen=false }: { diagnosis?: DiagnosisResult | null; onboardingAnswers?: OnboardingAnswers; onUpdateAirlinePreferences?:(answers:OnboardingAnswers)=>void; onEditDiagnosis?: () => void; onLogin:()=>void; initialAccountOpen?:boolean }) {
  ...
  if(mockReport&&!practiceConfig)return <MockInterviewReport session={mockReport} attempts={interviewAttempts} onBack={()=>{setMockReport(null);setMockSession(null);if(workspaceAirlineId){setTrainingView('airline-targeting');setActiveNav('home')}else setActiveNav('interview')}} onRetake={()=>{setMockReport(null);setMockInterviewOpen(true);setMockSession(null)}} onRetry={(attempt)=>{const question=resolveInterviewQuestion(attempt.questionId,attempt.targetAirlineId);if(question){setMockSession(null);startInterviewQuestion(question,attempt.id,undefined,attempt.targetAirlineId,undefined,{source:'mock_report',mockSessionId:mockReport.id,sourceAttemptId:attempt.id,questionId:question.id})}}} onAction={startMockReportAction}/>
