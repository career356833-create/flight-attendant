...
import { MockInterviewLauncher, MockInterviewProgress, completeSessionAttempt } from '@/components/interview-practice/mock-interview-session'
...
export function HomeDashboard({ diagnosis, onboardingAnswers, onEditDiagnosis, onLogin, initialAccountOpen=false }: { diagnosis?: DiagnosisResult | null; onboardingAnswers?: OnboardingAnswers; onEditDiagnosis?: () => void; onLogin:()=>void; initialAccountOpen?:boolean }) {
  ...
  function handleInterviewComplete(attempt:InterviewAttempt){setInterviewAttempts(loadInterviewAttempts());setCapabilityGains(loadInterviewCapabilityGains());if(mockSession){const next=completeSessionAttempt(mockSession,attempt.id);setMockSession(next);if(next.status==='in_progress'){const question=interviewQuestionById.get(next.questionIds[next.currentQuestionIndex]);if(question)setPracticeConfig({question,attemptType:'first',targetAirlineId:next.airlineId})}else setPracticeConfig(null)}}
  ...
  if(mockInterviewOpen)return <MockInterviewLauncher airlineId={diagnosis?.primaryAirline} onBack={()=>setMockInterviewOpen(false)} onStart={(session,question)=>{setMockSession(session);setMockInterviewOpen(false);setPracticeConfig({question,attemptType:'first',targetAirlineId:session.airlineId})}}/>
