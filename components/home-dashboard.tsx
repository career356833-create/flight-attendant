...
export function HomeDashboard({ diagnosis, onboardingAnswers, onUpdateAirlinePreferences, onEditDiagnosis, onLogin, initialAccountOpen=false }: { diagnosis?: DiagnosisResult | null; onboardingAnswers?: OnboardingAnswers; onUpdateAirlinePreferences?:(answers:OnboardingAnswers)=>void; onEditDiagnosis?: () => void; onLogin:()=>void; initialAccountOpen?:boolean }) {
  ...
  function startAirlineWorkspacePractice(airlineId:string,mode:'interview'|'mock'|'self_intro',question?:WorkspaceQuestion){setWorkspaceAirlineId(airlineId);if(mode==='self_intro'){setSelfIntroductionChallengeTarget(undefined);setTrainingView('self-introduction');return}if(mode==='mock'){setMockInterviewOpen(true);setTrainingView('dashboard');setActiveNav('interview');return}const fallback=interviewQuestionById.get(question?.category==='SAFETY'?'sj1':question?.category==='SERVICE'||question?.category==='CUSTOMER_COMPLAINT'?'cs1':question?.category==='EXPERIENCE'||question?.category==='TEAMWORK'?'be1':'im2');if(fallback){setTrainingView('dashboard');setActiveNav('interview');startInterviewQuestion(fallback,undefined,undefined,airlineId,undefined,question?{...buildPracticeLineage(airlineId,question),questionId:fallback.id}:{source:'airline_workspace',airlineId,questionId:fallback.id},undefined,'airline_workspace')}}
  ...
  function exitInterviewPractice(){const target=interviewReturnTarget;releaseMicrophoneCheckSession(mockSession?.id);setActiveWeeklyTask(null);setPracticeConfig(null);setInterviewReturnTarget('interview');if(target==='weekly-report'){setTrainingView('weekly-report');setActiveNav('home')}}
  ...
  if(trainingView==='airline-targeting')return featureShell(<AirlineTargetingWorkspace preferences={{primary:onboardingAnswers?.primaryAirline??null,interests:onboardingAnswers?.interestAirlines??[]}} onPreferencesChange={(preferences:AirlineTargetPreferences)=>{if(onboardingAnswers)onUpdateAirlinePreferences?.({...onboardingAnswers,primaryAirline:preferences.primary,interestAirlines:preferences.interests})}} onExit={()=>setTrainingView('dashboard')} onOpenApplication={airlineId=>{setApplicationAirlineId(airlineId);setTrainingView('application-coach')}} onOpenExperience={()=>setTrainingView('experience-library')} onStartPractice={startAirlineWorkspacePractice}/>)
  ...
  if(mockInterviewOpen)return <MockInterviewLauncher airlineId={workspaceAirlineId??diagnosis?.primaryAirline} onBack={()=>{setMockInterviewOpen(false);setWorkspaceAirlineId(undefined)}} onView={(session)=>{setMockInterviewOpen(false);setMockSession(session);setMockReport(session)}} onStart={(session,question)=>{const linked=activeWeeklyTask?{...session,weeklyTaskContext:activeWeeklyTask}:session;saveInterviewSession(linked);setMockSession(linked);setMockInterviewOpen(false);setWorkspaceAirlineId(undefined);setPracticeConfig({question,attemptType:'first',targetAirlineId:session.airlineId})}}/>
