...
import { releaseMicrophoneCheckSession } from '@/components/interview-practice/use-microphone-check'
...
import { linkExperienceToWorkDraft, listApplicationAnswers } from '@/lib/application-answer-repository'
...
import { buildMockTrainingLoopModel, buildTrainingLoopModel, type TrainingLoopAction } from '@/lib/training-loop'
...
import { AirlineTargetingWorkspace } from '@/components/airline-targeting'
import { buildPracticeLineage, resolveWorkspaceInterviewQuestion, workspaceQuestionToInterviewQuestion, type AirlineTargetPreferences, type WorkspaceQuestion } from '@/lib/airline-targeting-workspace'
import { airlineJourneyContextFromQuestion, genericAirlineJourneyContext, journeyContextForExperience, journeyContextFromPracticeLineage, journeyContextWithExperience, type AirlineJourneyContext } from '@/lib/airline-journey-context'
...
export function HomeDashboard({ diagnosis, onboardingAnswers, onUpdateAirlinePreferences, onEditDiagnosis, onLogin, initialAccountOpen=false }: { diagnosis?: DiagnosisResult | null; onboardingAnswers?: OnboardingAnswers; onUpdateAirlinePreferences?:(answers:OnboardingAnswers)=>void; onEditDiagnosis?: () => void; onLogin:()=>void; initialAccountOpen?:boolean }) {
  const [activeNav, setActiveNav] = useState(initialAccountOpen?'my':'home')
  const [trainingView,setTrainingView]=useState<'dashboard'|'self-introduction'|'experience-library'|'application-coach'|'application-tracker'|'weekly-report'|'competency-assessment'|'airline-targeting'>('dashboard')
  const [competencyVideoQuestion,setCompetencyVideoQuestion]=useState<VideoQuestion>()
  const [applicationAirlineId,setApplicationAirlineId]=useState<string>()
  const [workspaceAirlineId,setWorkspaceAirlineId]=useState<string>()
  const [airlineJourneyContext,setAirlineJourneyContext]=useState<AirlineJourneyContext>()
  const [experienceJourneyContext,setExperienceJourneyContext]=useState<AirlineJourneyContext>()
  ...
  const [weeklyResultReturn,setWeeklyResultReturn]=useState<WeeklyResultReturn|null>(null)
  ...
  // Training Loop V2: derived from the records the feature repositories already hold; nothing extra is stored.
  function buildInterviewTrainingLoop(current:InterviewAttempt){
    return buildTrainingLoopModel({attempt:current,attempts:interviewAttempts.filter(item=>item.id!==current.id),selfIntroductions:loadSelfIntroductionAttempts(),sessions:interviewSessions,applicationAnswerCount:listApplicationAnswers().length,queue:interviewPracticeQueueRepository.listQueue(),favorites:interviewPracticeQueueRepository.listFavorites(),journeyAction:preparationJourney?.nextRecommendedAction??null,dailyFallback:dailyPlan.primary?.target})
  }
  // Retakes are handled inside the practice engine (it restarts the same question in place); only the
  // "next practice" hand-off reaches the dashboard, and it reuses the existing navigation.
  function startTrainingLoopAction(action:TrainingLoopAction){
    startDailyAction({id:`training-loop:${action.kind}`,dedupeKey:`training-loop:${action.kind}`,type:'fallback',title:action.label,description:'',reason:action.reason,priority:0,target:action.target,source:'airline_target',resume:false})
  }
  ...
  function retakeSingleInterview(attempt:InterviewAttempt){const lineageAirlineId=attempt.targetAirlineId??attempt.sourceContext?.airlineId;const question=resolveInterviewQuestion(attempt.questionId,lineageAirlineId);if(!question)return;const language=attempt.speechMetrics?.language==='en'?'en':attempt.speechMetrics?.language==='ko'?'ko':undefined;/* Airline lineage is carried forward as-is; AI context stays gated downstream by getApplicationAirlineContext. */const safeAirlineId=lineageAirlineId;if(attempt.sourceContext?.source==='airline_workspace'&&lineageAirlineId){setWorkspaceAirlineId(lineageAirlineId);setAirlineJourneyContext(journeyContextFromPracticeLineage(attempt.sourceContext,resolveWorkspaceInterviewQuestion(attempt.questionId,lineageAirlineId)?.prompt))}const safeExperienceId=attempt.experienceId&&experienceRepository.load().experiences.some(item=>item.id===attempt.experienceId)?attempt.experienceId:undefined,source:SingleInterviewResumeSource=attempt.weeklyTaskContext?'weekly_task':attempt.sourceContext?.source??'direct';const config:InterviewPracticeConfig={question,attemptType:'retry',previousAttemptId:attempt.id,targetAirlineId:safeAirlineId,selectedExperienceId:safeExperienceId,sourceContext:attempt.sourceContext,weeklyTaskContext:attempt.weeklyTaskContext,languageHint:language};setInterviewReturnTarget(interviewReturnTargetForSource(source));setPracticeConfig(config);singleInterviewResumeRepository.save(resumeFromConfig(config,source))}
  function resumeSingleInterview(){const stored=singleInterviewResumeRepository.load();if(!stored)return;const restored=restoreSingleInterviewConfig(stored,{questionExists:id=>Boolean(resolveInterviewQuestion(id,stored.targetAirlineId??stored.sourceContext?.airlineId)),airlineContextAllowed,experienceExists:id=>experienceRepository.load().experiences.some(item=>item.id===id)});const question=resolveInterviewQuestion(stored.questionId,stored.targetAirlineId??stored.sourceContext?.airlineId);if(!restored||!question){setInvalidResume(true);return}setInvalidResume(false);setMockSession(null);setActiveWeeklyTask(restored.weeklyTaskContext??null);setInterviewReturnTarget(interviewReturnTargetForSource(stored.source));setActiveNav('interview');setPracticeConfig({question,...restored})}
  ...
  function startAirlineWorkspacePractice(airlineId:string,mode:'interview'|'mock'|'self_intro',question?:WorkspaceQuestion){setWorkspaceAirlineId(airlineId);setAirlineJourneyContext(question?airlineJourneyContextFromQuestion(question,'airline_workspace','questions'):genericAirlineJourneyContext(airlineId,'practice'));if(mode==='self_intro'){setSelfIntroductionChallengeTarget(undefined);setTrainingView('self-introduction');return}if(mode==='mock'){setMockInterviewOpen(true);setTrainingView('dashboard');setActiveNav('interview');return}const workspacePractice=question?workspaceQuestionToInterviewQuestion(question):null;if(workspacePractice&&question){setTrainingView('dashboard');setActiveNav('interview');startInterviewQuestion(workspacePractice,undefined,undefined,airlineId,undefined,{...buildPracticeLineage(airlineId,question),questionId:workspacePractice.id},undefined,'airline_workspace','airline-workspace');return}const fallback=interviewQuestionById.get(question?.category==='SAFETY'?'sj1':question?.category==='SERVICE'||question?.category==='CUSTOMER_COMPLAINT'?'cs1':question?.category==='EXPERIENCE'||question?.category==='TEAMWORK'?'be1':'im2');if(fallback){setTrainingView('dashboard');setActiveNav('interview');startInterviewQuestion(fallback,undefined,undefined,airlineId,undefined,question?{...buildPracticeLineage(airlineId,question),questionId:fallback.id}:{source:'airline_workspace',airlineId,questionId:fallback.id},undefined,'airline_workspace','airline-workspace')}}
  ...
  function completeWeeklyTask(event:Parameters<typeof completeWeeklyTaskContext>[1],override?:WeeklyTaskContext){const context=override??activeWeeklyTask;if(!context||!completeWeeklyTaskContext(context,event))return false;setLearningRevision(value=>value+1);setWeeklyResultReturn({context,entityId:event.entityId});setActiveWeeklyTask(null);return true}
  function returnToWeeklyReport(){setWeeklyResultReturn(null);setActiveWeeklyTask(null);setTrainingView('weekly-report');setActiveNav('home')}
  function startDailyAction(action:DailyActionCandidate,explicitWeeklyContext?:WeeklyTaskContext|null,interviewSource:SingleInterviewResumeSource='daily_plan',returnTarget?:InterviewPracticeReturnTarget){
    ...
    if(target.kind==='airline_workspace'){setWorkspaceAirlineId(target.airlineId);setTrainingView('airline-targeting');setActiveNav('home');return}
  ...
  function handleTaskStart(id:string){setWeeklyResultReturn(null);if(id===SELF_INTRO_TASK_ID){setSelfIntroductionChallengeTarget(undefined);setTrainingView('self-introduction');return} if(id==='application-coach-motivation'){setActiveNav('resume');setTrainingView('application-coach');return} if(id.startsWith('interview-question-')){const q=interviewQuestionById.get(id.replace('interview-question-',''));if(q){setActiveNav('interview');startInterviewQuestion(q)}return} toggleTask(id) }
  ...
  function exitInterviewPractice(){const target=interviewReturnTarget;releaseMicrophoneCheckSession(mockSession?.id);setActiveWeeklyTask(null);setPracticeConfig(null);setInterviewReturnTarget('interview');if(target==='weekly-report'){setTrainingView('weekly-report');setActiveNav('home')}else if(target==='airline-workspace'){setTrainingView('airline-targeting');setActiveNav('home')}}
  function exitSelfIntroductionAndNavigate(target:string){const transition=exitSelfIntroductionForNavigation(target as PrimaryNavigationTarget);setWeeklyResultReturn(null);if(transition.clearActiveWeeklyTask)setActiveWeeklyTask(null);if(transition.clearChallengeTarget)setSelfIntroductionChallengeTarget(undefined);setTrainingView(transition.trainingView);setActiveNav(transition.activeNav)}
  function advanceMockSession(session:InterviewSession,attempt:InterviewAttempt){const next=completeInterviewSessionAttempt(session,attempt.id);if(next.status==='completed'){releaseMicrophoneCheckSession(session.id);const completed={...next,sessionAnalysis:analyzeInterviewSession(next,loadInterviewAttempts())};const saved=saveInterviewSession(completed);if(saved.ok)completeWeeklyTask({type:'mock_session',entityId:completed.id,completed:true},completed.weeklyTaskContext);setMockSession(completed);setMockReport(completed);setPracticeConfig(null);return}saveInterviewSession(next);setMockSession(next);const question=interviewQuestionById.get(next.questionIds[next.currentQuestionIndex]);if(question)setPracticeConfig({question,attemptType:'first',targetAirlineId:next.airlineId})}
  function handleInterviewComplete(attempt:InterviewAttempt){if(!mockSession)singleInterviewResumeRepository.clear();if(practiceConfig?.sourceQueueItemId){const practiced=interviewPracticeQueueRepository.markPracticed(practiceConfig.sourceQueueItemId);if(practiced)completeWeeklyTask({type:'queue_practiced',entityId:practiceConfig.sourceQueueItemId,completed:true},attempt.weeklyTaskContext)}else if(!mockSession&&completeWeeklyInterviewAttempt(attempt)){setLearningRevision(value=>value+1);setActiveWeeklyTask(null)}setInterviewAttempts(loadInterviewAttempts());if(!mockSession)return;if(pendingFollowUp){const enriched=pendingFollowUp.adaptive?{...attempt,isFollowUp:true,parentAttemptId:pendingFollowUp.attempt.id,followUpReason:pendingFollowUp.reason,followUpTemplateId:pendingFollowUp.templateId}:attempt;if(pendingFollowUp.adaptive){const saved=saveInterviewAttempt(enriched);if(saved.ok)queueTrainingAttempt('interview',enriched,Boolean(enriched.audioId));setInterviewAttempts(loadInterviewAttempts())}setPendingFollowUp(null);advanceMockSession(mockSession,enriched);return}setPendingSessionAttempt(attempt)}
  ...
  /* Leaves a feature view through the sidebar: resets the feature's transient journey state, then shows the requested top-level tab. */
  const leaveFeatureView=(target:string)=>{setWeeklyResultReturn(null);if(!practiceConfig)setActiveWeeklyTask(null);setSelectedInterviewResult(null);setApplicationAirlineId(undefined);setExperienceJourneyContext(undefined);setAirlineJourneyContext(undefined);setWorkspaceAirlineId(undefined);setTrainingView('dashboard');setActiveNav(target as typeof activeNav)}
  ...
  const weeklyReturnEntityId=weeklyResultReturn?.entityId
  if(selectedInterviewResult){return featureShell(<SingleInterviewResultRevisit attempt={selectedInterviewResult} onBack={()=>setSelectedInterviewResult(null)} onRetake={()=>{setSelectedInterviewResult(null);setActiveNav('interview');retakeSingleInterview(selectedInterviewResult)}}/>,leaveFeatureView)}
  if(competencyVideoQuestion)return featureShell(<SelfIntroductionFlow assessmentVideo={{id:competencyVideoQuestion.id,prompt:competencyVideoQuestion.prompt,recommendedSeconds:competencyVideoQuestion.recommendedSeconds as 30|60|90}} onAssessmentVideoComplete={attempt=>attachCompetencyVideoAttempt(competencyVideoQuestion.id,attempt)} targetAirlineId={diagnosis?.primaryAirline} onExit={()=>setCompetencyVideoQuestion(undefined)} onComplete={()=>{}}/>,target=>{setCompetencyVideoQuestion(undefined);setTrainingView(target==='weekly-report'?'weekly-report':'competency-assessment')})
  if(trainingView==='competency-assessment')return featureShell(<CompetencyAssessment onExit={()=>setTrainingView('dashboard')} onStartVideo={setCompetencyVideoQuestion} onPractice={destination=>{if(destination==='self_introduction'){setTrainingView('self-introduction');return}if(destination==='weekly_report'){setTrainingView('weekly-report');return}setTrainingView('dashboard');setActiveNav('interview')}}/>,target=>setTrainingView(target==='weekly-report'?'weekly-report':'dashboard'))
  if(trainingView==='self-introduction')return featureShell(<SelfIntroductionFlow onTrainingLoopNext={startTrainingLoopAction} targetAirlineId={workspaceAirlineId??diagnosis?.primaryAirline} initialChallengeTarget={selfIntroductionChallengeTarget} weeklyReturnAttemptId={isWeeklyResultReturnEligible(weeklyResultReturn,'self_introduction',weeklyReturnEntityId)?weeklyReturnEntityId:undefined} onWeeklyReturn={returnToWeeklyReport} onExit={()=>{setWeeklyResultReturn(null);setActiveWeeklyTask(null);setSelfIntroductionChallengeTarget(undefined);if(workspaceAirlineId){setTrainingView('airline-targeting');setActiveNav('home')}else setTrainingView('dashboard')}} onComplete={handleTrainingComplete}/>,exitSelfIntroductionAndNavigate)
  if(trainingView==='experience-library')return featureShell(<ExperienceLibrary journeyContext={experienceJourneyContext} onSelectForJourney={(experience,context)=>{const linked=linkExperienceToWorkDraft(context,experience.id);/* Always return to the coach: with the experience linked when the draft was found, otherwise to the same draft context so the user can pick it there. */setAirlineJourneyContext(linked.ok?journeyContextWithExperience(context,experience.id):context);setExperienceJourneyContext(undefined);setApplicationAirlineId(context.airlineId);setTrainingView('application-coach');return linked.ok}} weeklyReturnExperienceId={isWeeklyResultReturnEligible(weeklyResultReturn,'experience',weeklyReturnEntityId)?weeklyReturnEntityId:undefined} onWeeklyReturn={returnToWeeklyReport} onExit={()=>{setWeeklyResultReturn(null);if(!practiceConfig)setActiveWeeklyTask(null);if(experienceJourneyContext?.origin==='application'){setAirlineJourneyContext(experienceJourneyContext);setApplicationAirlineId(experienceJourneyContext.airlineId);setExperienceJourneyContext(undefined);setTrainingView('application-coach')}else{setExperienceJourneyContext(undefined);setTrainingView(workspaceAirlineId?'airline-targeting':'dashboard')}}} onPractice={(question,experience)=>{setWeeklyResultReturn(null);setActiveWeeklyTask(null);setTrainingView('dashboard');setActiveNav('interview');startInterviewQuestion(question,undefined,experience.id,workspaceAirlineId,undefined,workspaceAirlineId?{source:'airline_workspace',airlineId:workspaceAirlineId,questionId:question.id}:undefined,undefined,workspaceAirlineId?'airline_workspace':'direct')}}/>,leaveFeatureView)
  ...
  if(trainingView==='application-coach'||activeNav==='resume')return featureShell(<ApplicationCoach initialAirlineId={applicationAirlineId??(onboardingAnswers?.primaryAirline&&!['custom_airline','undecided_airline'].includes(onboardingAnswers.primaryAirline.id)?onboardingAnswers.primaryAirline.id:undefined)} initialJourneyContext={airlineJourneyContext} weeklyReturnAnswerId={isWeeklyResultReturnEligible(weeklyResultReturn,'application',weeklyReturnEntityId)?weeklyReturnEntityId:undefined} onWeeklyReturn={returnToWeeklyReport} onExit={()=>{setWeeklyResultReturn(null);setActiveWeeklyTask(null);setApplicationAirlineId(undefined);setTrainingView(workspaceAirlineId?'airline-targeting':'dashboard');setActiveNav('home')}} onOpenExperience={context=>{setExperienceJourneyContext(context?journeyContextForExperience(context):undefined);setTrainingView('experience-library')}} onPractice={startApplicationDrill} onTrainingLoopNext={startTrainingLoopAction}/>,leaveFeatureView)
  ...
  if(trainingView==='airline-targeting')return featureShell(<AirlineTargetingWorkspace preferences={{primary:onboardingAnswers?.primaryAirline??null,interests:onboardingAnswers?.interestAirlines??[]}} applications={trackedApplications} initialAirlineId={workspaceAirlineId} initialJourneyContext={airlineJourneyContext} onPreferencesChange={(preferences:AirlineTargetPreferences)=>{if(onboardingAnswers)onUpdateAirlinePreferences?.({...onboardingAnswers,primaryAirline:preferences.primary,interestAirlines:preferences.interests})}} onExit={()=>{setWorkspaceAirlineId(undefined);setAirlineJourneyContext(undefined);setTrainingView('dashboard')}} onOpenApplication={context=>{setWorkspaceAirlineId(context.airlineId);setAirlineJourneyContext(context);setApplicationAirlineId(context.airlineId);setTrainingView('application-coach')}} onOpenExperience={context=>{setWorkspaceAirlineId(context.airlineId);setAirlineJourneyContext(context);setExperienceJourneyContext(undefined);setTrainingView('experience-library')}} onStartPractice={startAirlineWorkspacePractice}/>)
  if(practiceConfig)return <>{mockSession&&<MockInterviewProgress session={mockSession} onExit={exitInterviewPractice}/>}<InterviewPracticeEngine key={`${mockSession?.id??'single'}:${practiceConfig.question.id}:${practiceConfig.followUp?.parentAttemptId??'base'}`} config={practiceConfig} onExit={exitInterviewPractice} onComplete={handleInterviewComplete} onNextQuestion={(question)=>startInterviewQuestion(question)} onOpenExperience={selectedExperienceId=>{setPracticeConfig(current=>current?{...current,selectedExperienceId}:current);setTrainingView('experience-library')}} buildTrainingLoop={buildInterviewTrainingLoop} onTrainingLoopAction={startTrainingLoopAction} micSessionKey={mockSession?.id} sessionAction={mockSession&&pendingSessionAttempt?{label:mockSession.currentQuestionIndex>=mockSession.questionIds.length-1?'모의면접 결과 보기':'다음 질문',onContinue:continueMockSession}:undefined}/></>
  ...
  if(mockReport&&!practiceConfig)return <MockInterviewReport trainingLoop={buildMockTrainingLoopModel({session:mockReport,sessions:interviewSessions,attempts:interviewAttempts,selfIntroductions:loadSelfIntroductionAttempts(),applicationAnswerCount:listApplicationAnswers().length,queue:interviewPracticeQueueRepository.listQueue(),journeyAction:preparationJourney?.nextRecommendedAction??null,dailyFallback:dailyPlan.primary?.target})} onTrainingLoopAction={action=>{/* A retake reopens the mock launcher so the same configuration is chosen again. */if(action.kind==='retake'||action.kind==='focused_retake'){setMockReport(null);setMockSession(null);setMockInterviewOpen(true);return}startTrainingLoopAction(action)}} session={mockReport} attempts={interviewAttempts} onBack={()=>{setMockReport(null);setMockSession(null);if(workspaceAirlineId){setTrainingView('airline-targeting');setActiveNav('home')}else setActiveNav('interview')}} onRetake={()=>{setMockReport(null);setMockInterviewOpen(true);setMockSession(null)}} onRetry={(attempt)=>{const question=resolveInterviewQuestion(attempt.questionId,attempt.targetAirlineId);if(question){setMockSession(null);startInterviewQuestion(question,attempt.id,undefined,attempt.targetAirlineId,undefined,{source:'mock_report',mockSessionId:mockReport.id,sourceAttemptId:attempt.id,questionId:question.id})}}} onAction={startMockReportAction}/>
  if(mockInterviewOpen)return <MockInterviewLauncher airlineId={workspaceAirlineId??diagnosis?.primaryAirline} onBack={()=>{setMockInterviewOpen(false);if(workspaceAirlineId){setTrainingView('airline-targeting');setActiveNav('home')}}} onView={(session)=>{setMockInterviewOpen(false);setMockSession(session);setMockReport(session)}} onStart={(session,question)=>{const linked=activeWeeklyTask?{...session,weeklyTaskContext:activeWeeklyTask}:session;saveInterviewSession(linked);setMockSession(linked);setMockInterviewOpen(false);setPracticeConfig({question,attemptType:'first',targetAirlineId:session.airlineId})}}/>
