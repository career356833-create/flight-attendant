...
export function HomeDashboard({ diagnosis, onboardingAnswers, onEditDiagnosis, onLogin, initialAccountOpen=false }: { diagnosis?: DiagnosisResult | null; onboardingAnswers?: OnboardingAnswers; onEditDiagnosis?: () => void; onLogin:()=>void; initialAccountOpen?:boolean }) {
  ...
  function startInterviewQuestion(question:InterviewQuestion,previousAttemptId?:string,selectedExperienceId?:string,targetAirlineOverride?:string,sourceQueueItemId?:string,sourceContext?:InterviewPracticeSourceContext,weeklyTaskContext?:WeeklyTaskContext,source?:SingleInterviewResumeSource){const latest=interviewAttempts.find(attempt=>attempt.questionId===question.id);const linkedId=previousAttemptId??latest?.id;const config:InterviewPracticeConfig={question,attemptType:linkedId?'retry':'first',previousAttemptId:linkedId,targetAirlineId:targetAirlineOverride??diagnosis?.primaryAirline,selectedExperienceId,sourceQueueItemId,sourceContext,weeklyTaskContext};setPracticeConfig(config);singleInterviewResumeRepository.save(resumeFromConfig(config,source??(sourceContext?.source??(sourceQueueItemId?'queue':weeklyTaskContext?'weekly_task':'direct'))))}
  function retakeSingleInterview(attempt:InterviewAttempt){const question=interviewQuestionById.get(attempt.questionId);if(!question)return;const language=attempt.speechMetrics?.language==='en'?'en':attempt.speechMetrics?.language==='ko'?'ko':undefined;const safeAirlineId=attempt.targetAirlineId&&airlineContextAllowed(attempt.targetAirlineId)?attempt.targetAirlineId:undefined;const safeExperienceId=attempt.experienceId&&experienceRepository.load().experiences.some(item=>item.id===attempt.experienceId)?attempt.experienceId:undefined;const config:InterviewPracticeConfig={question,attemptType:'retry',previousAttemptId:attempt.id,targetAirlineId:safeAirlineId,selectedExperienceId:safeExperienceId,sourceContext:attempt.sourceContext,languageHint:language};setPracticeConfig(config);singleInterviewResumeRepository.save(resumeFromConfig(config,attempt.sourceContext?.source??'direct'))}
