...
import { MockInterviewFollowUp, MockInterviewLauncher, MockInterviewProgress, MockInterviewReport } from '@/components/interview-practice/mock-interview-session'
...
export function HomeDashboard({ diagnosis, onboardingAnswers, onEditDiagnosis, onLogin, initialAccountOpen=false }: { diagnosis?: DiagnosisResult | null; onboardingAnswers?: OnboardingAnswers; onEditDiagnosis?: () => void; onLogin:()=>void; initialAccountOpen?:boolean }) {
  ...
  function advanceMockSession(session:InterviewSession,attempt:InterviewAttempt){const next=completeInterviewSessionAttempt(session,attempt.id);if(next.status==='completed'){const completed={...next,sessionAnalysis:analyzeInterviewSession(next,[...loadInterviewAttempts(),attempt])};saveInterviewSession(completed);setMockSession(completed);setMockReport(completed);setPracticeConfig(null);return}saveInterviewSession(next);setMockSession(next);const question=interviewQuestionById.get(next.questionIds[next.currentQuestionIndex]);if(question)setPracticeConfig({question,attemptType:'first',targetAirlineId:next.airlineId})}
  function handleInterviewComplete(attempt:InterviewAttempt){setInterviewAttempts(loadInterviewAttempts());setCapabilityGains(loadInterviewCapabilityGains());if(!mockSession)return;if(pendingFollowUp){setPendingFollowUp(null);advanceMockSession(mockSession,attempt);return}const message=followUpForAttempt(attempt);if(message){const held=completeInterviewSessionAttempt(mockSession,attempt.id,false);saveInterviewSession(held);setMockSession(held);setPracticeConfig(null);setPendingFollowUp({attempt,message});return}advanceMockSession(mockSession,attempt)}
  ...
  if(practiceConfig)return <>{mockSession&&<MockInterviewProgress session={mockSession} onExit={()=>setPracticeConfig(null)}/>}<InterviewPracticeEngine config={practiceConfig} onExit={()=>setPracticeConfig(null)} onComplete={handleInterviewComplete} onNextQuestion={(question)=>startInterviewQuestion(question)}/></>
