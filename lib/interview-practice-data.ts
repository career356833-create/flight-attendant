...
import type { ContentAnalysisState, TranscriptProvenance, TranscriptReview } from '@/lib/speech-understanding-v2'
...
export type TranscriptIntegrity={providerId?:string;mode:TranscriptionMode|'unknown';isActualTranscription:boolean;transcriptProvenance?:TranscriptProvenance}
export type InterviewPracticeSourceContext = { source:'application_drill'|'mock_report'|'airline_workspace'; applicationAnswerId?:string; applicationVersionId?:string; weakness?:string; mockSessionId?:string; sourceAttemptId?:string; questionId?:string; workspaceQuestionId?:string; airlineId?:string; questionKind?:'APPLICATION'|'INTERVIEW'; questionSourceType?:string; questionProvenance?:'OFFICIAL_CURRENT'|'OFFICIAL_ARCHIVE'|'USER_REPORTED'|'VERIFIED_SECONDARY'|'UNVERIFIED_COMMUNITY' }
export type InterviewAttempt = { id:string; questionId:string; category:InterviewCategory; createdAt:string; transcript:string; transcriptIntegrity?:TranscriptIntegrity; transcriptReview?:TranscriptReview; contentAnalysisState?:ContentAnalysisState; durationSeconds:number; audioId?:string; audioPath?:string; audioMetrics?:InterviewAudioMetrics;speechMetrics?:InterviewSpeechMetrics;pronunciationAnalysis?:PronunciationAnalysisResult;contentAnalysis?:InterviewContentAnalysis; analysis:InterviewAnswerAnalysis; targetAirlineId?:string; experienceId?:string; experienceSnapshot?:{title:string;category:ExperienceCategory;shortSummary:string}; sourceContext?:InterviewPracticeSourceContext; weeklyTaskContext?:WeeklyTaskContext; attemptNumber:number; previousAttemptId?:string; isFollowUp?:boolean; parentAttemptId?:string; followUpReason?:string; followUpTemplateId?:string; completed:boolean }
...
export type InterviewPracticeStep = 'intro'|'microphone_check'|'recording'|'review'|'analyzing'|'transcript_review'|'content_analyzing'|'result'|'retry'
...
export function saveInterviewAttempt(attempt:InterviewAttempt){const list=loadInterviewAttempts();return safeLocalStorageWrite(ATTEMPTS_KEY,[attempt,...list.filter(a=>a.id!==attempt.id)].slice(0,100),{category:'interview_attempt'})}
...
export function recordInterviewProgress(attempt:InterviewAttempt){if(typeof window==='undefined')return{ok:false as const,reason:'storage_unavailable' as const};try{const data=JSON.parse(localStorage.getItem(PROGRESS_KEY)??'{"dailyGains":{},"questionGains":{},"capabilityGains":{}}');const day=new Date().toISOString().slice(0,10);const dayGain=Number(data.dailyGains?.[day]??0);const questionGain=Number(data.questionGains?.[attempt.questionId]??0);const previous=attempt.previousAttemptId?loadInterviewAttempts().find(a=>a.id===attempt.previousAttemptId):undefined;const delta=questionGain===0?1:previous&&attempt.analysis.overallScore>=previous.analysis.overallScore+5&&questionGain<2?1:0;const applied=dayGain>=3?0:delta;data.dailyGains={...data.dailyGains,[day]:dayGain+applied};data.questionGains={...data.questionGains,[attempt.questionId]:questionGain+applied};data.capabilityGains=data.capabilityGains??{};for(const key of interviewQuestionById.get(attempt.questionId)?.targetCapabilities??[])data.capabilityGains[key]=Number(data.capabilityGains[key]??0)+applied;return safeLocalStorageWrite(PROGRESS_KEY,data,{category:'learning'})}catch{return{ok:false as const,reason:'serialization_failed' as const}}}
