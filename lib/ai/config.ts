...
export function loadAiConfig():AiProviderConfig{if(typeof window==='undefined')return DEFAULT_AI_CONFIG;try{return normalizeStoredAiConfig(JSON.parse(localStorage.getItem(KEY)??'{}'),process.env.NODE_ENV==='production')}catch{return DEFAULT_AI_CONFIG}}
export function saveAiConfig(config:AiProviderConfig){if(typeof window!=='undefined')localStorage.setItem(KEY,JSON.stringify({...config,actualSttV1:true}));return config}
