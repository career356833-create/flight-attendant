...
export type TranscriptionMode='actual_audio'|'fallback'|'mock'|'unavailable'
export type TranscriptionResult={transcript:string;detectedLanguage?:string;confidence?:number|null;segments?:TranscriptionSegment[];fillerWords?:{word:string;count:number}[];silenceSegments?:{startSeconds:number;endSeconds:number}[];transcriptionMode?:TranscriptionMode;isActualTranscription?:boolean}
