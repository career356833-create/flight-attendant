...
test('all fourteen current-week tasks have real execution destinations',()=>{const actions=currentDays().flatMap(day=>day.tasks).map(task=>weeklyTaskToDailyAction(task,{validQuestionIds:['im2'],balancedQuestionId:'im2'}));assert.equal(actions.length,14);assert.equal(actions.every(Boolean),true)})
test('current-week kind distribution remains deterministic',()=>{const counts=Object.groupBy(currentDays().flatMap(day=>day.tasks),task=>task.type);assert.deepEqual(Object.fromEntries(Object.entries(counts).map(([kind,tasks])=>[kind,tasks?.length])),{interview_question:2,review:9,experience_work:1,self_introduction:1,application_work:1})})
