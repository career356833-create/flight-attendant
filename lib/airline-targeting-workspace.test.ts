...
test("patterns exclude non-official stored questions", () => assert.deepEqual(calculateQuestionPatterns([question("1", "APPLICATION"), question("2", "INTERVIEW")]), []));
...
test("practice lineage preserves airline, question, source and provenance", () => assert.deepEqual(buildPracticeLineage("a", question("q", "INTERVIEW")), { source: "airline_workspace", airlineId: "a", workspaceQuestionId: "q", questionKind: "INTERVIEW", questionSourceType: "USER_REPORTED", questionProvenance: "USER_REPORTED" }));
...
test("master profiles do not invent operation scope beyond explicit official overrides", () => {
  const explicitOfficialScopes = new Set(["jeju_air", "jin_air", "tway_air", "air_busan", "air_seoul", "eastar_jet", "emirates", "qatar_airways", "etihad_airways", "singapore_airlines", "cathay_pacific", "ana", "japan_airlines", "turkish_airlines", "lufthansa", "british_airways", "air_france", "klm", "delta_air_lines", "united_airlines", "american_airlines", "air_canada"]);
